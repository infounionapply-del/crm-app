...
export const NAV_ITEMS: Record<string, NavItem> = {
  ...
  priceList: { id: 'priceList', path: '/price-list', icon: Tags, label: 'nav.price_list', roles: ['Manager', 'Admin', 'Super Admin', 'Administrator', 'Support'] },
  approvals: { id: 'approvals', path: '/approvals', icon: CheckSquare, label: 'nav.approvals', roles: ['Manager', 'Admin', 'Super Admin', 'Administrator'] },
  reports: { id: 'reports', path: '/reports', icon: FileText, label: 'nav.reports' }, // Visible to all
  targets: { id: 'targets', path: '/targets', icon: Target, label: 'nav.sales_targets', roles: ['Manager', 'Admin', 'Super Admin', 'Administrator'] },
  ...
  return [
    {
      ...
      items: [NAV_ITEMS.home, NAV_ITEMS.customers, NAV_ITEMS.opportunities].filter(item => 
    ...
    },
    {
      ...
      items: [NAV_ITEMS.settings].filter(item => 
        (!item.roles || item.roles.includes(userRole)) && !item.excludeRoles?.includes(userRole)
      )
    }
...
export const getMobileBottomNav = (userRole: string): NavItem[] => {
  const isManager = ['Manager', 'Admin', 'Super Admin', 'Administrator'].includes(userRole);
  const isSupport = userRole === 'Support';

  let items: NavItem[] = [];
  if (isSupport) {
    items = [NAV_ITEMS.home, NAV_ITEMS.quotation, NAV_ITEMS.priceList];
  } else if (isManager) {
    items = [NAV_ITEMS.home, NAV_ITEMS.approvals, NAV_ITEMS.customers, NAV_ITEMS.opportunities];
  } else {
    // Default to Sales
    items = [NAV_ITEMS.home, NAV_ITEMS.customers, NAV_ITEMS.checkIn, NAV_ITEMS.opportunities];
  }
...
export const SETTINGS_SUBMENUS = [
  ...
  { path: '/settings?tab=company', icon: Building, label: 'nav.company_details', roles: ['Manager', 'Admin', 'Super Admin', 'Administrator'] },
  { path: '/settings?tab=users', icon: Users, label: 'nav.user_management', roles: ['Manager', 'Admin', 'Super Admin', 'Administrator'] },
  { path: '/settings?tab=notifications', icon: Bell, label: 'nav.notifications' },
  { path: '/settings?tab=pdf', icon: FileText, label: 'nav.pdf_templates', roles: ['Manager', 'Admin', 'Super Admin', 'Administrator'] },
];
